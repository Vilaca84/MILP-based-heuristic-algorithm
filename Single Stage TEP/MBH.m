...
for kapa = 1:MBH_trial
    ...
    while count1<countMAX || count2<countMAX
        ...
        if p1 == 0
            [s1, p1, i1] = f(s1, slt, pfi, 0, equipment, tsystem, voll, Xmax, Xmin, inv);
        else
            [s1, p1, i1] = f(s1, sut, pif, 1, equipment, tsystem, voll, Xmax, Xmin, inv);
        end
        
        % Lines 9 to 13 in Algorithm 1
        if p2 ~= 0
            [s2, p2, i2] = f(s2, sut, pif, 1, equipment, tsystem, voll, Xmax, Xmin, inv);
        else
